...
function getTargetSpreadsheet() {
  ...
  CACHED_SS = SPREADSHEET_URL? SpreadsheetApp.openByUrl(SPREADSHEET_URL): SpreadsheetApp.getActiveSpreadsheet();
  return CACHED_SS;
}
...
function getClassConfigurations() {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    data[date].push({group: group,imageUrl: imageUrl,maxSerial:isNaN(maxSerial)? null: maxSerial});
  }
  return data;
}
...
function checkDuplicate(sheet, roll, serial) {
  ...
  if (lastRow < ATTENDANCE_START_ROW) {
    return null;
  }
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (oldRoll.toLowerCase() ===roll.toLowerCase()) {
      return {type: "roll",message:"Roll Number " +roll +" has already submitted attendance. " +"Email ID: " +oldEmail};
    }
    // ------------------------------------------------------
    // Duplicate Seat
    // ------------------------------------------------------
    if (oldSerial === serial) {
      return {type: "seat",message:"Seat #" +serial +" has already been claimed by Roll Number " +oldRoll +". Email ID: " +oldEmail};
    }
  }
  return null;
}
...
function getClassConfig(ss, date, group) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    if (configDate === date &&configGroup === group) {
      ...
      return {date: configDate,group: configGroup,imageUrl: String(values[i][CONFIG_IMAGE_URL_COLUMN - 1] || "").trim(),maxSerial: maxSerial};
    }
  }
  return null;
}
